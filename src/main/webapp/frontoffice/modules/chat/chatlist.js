...
function getConversasList() {
    $.ajax({
            ...
            data: {'pwhat': 'findAllConversasForUser', 'userId': localStorage.getItem("userId") },
...
function getMensagensList() {
    $.ajax({
            ...
            success: function (data) {
                ...
                for (var i=0; i<count; i++) {
                    if (json.conversas[i].user.user_id == localStorage.getItem("userId")) {
                           console.log(json.conversas[i].user.user_id == localStorage.getItem("userId"));
...
function submitMensagem(){
            $.ajax({
                url: 'http://localhost:8080/prochild/ChatController',
                data: {'pwhat': 'sendMensagem', "userId": localStorage.getItem("userId"), "message": document.getElementById("mensagem").value, "conversaId": localStorage.getItem("id")},
                beforeSend: function (xhr) {                
